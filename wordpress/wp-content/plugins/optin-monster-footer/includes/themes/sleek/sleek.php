 ...
 */
class Optin_Monster_Footer_Theme_Sleek extends Optin_Monster_Theme {
    ...
    public function get_html() {
        ...
        $html = '<div id="om-' . $this->type . '-' . $this->theme . '-optin" class="om-' . $this->type . '-' . $this->theme . ' om-clearfix om-theme-' . $this->theme . ' ' . ( $provider && 'custom' == $provider ? 'om-custom-html-form' : '' ) . '">';
            $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-body" style="background-color: ' . $this->get_background_setting( 'body', '#000' ) . '">';
                // Optin text
                $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-title" data-om-action="editable" data-om-field="title" style="' . $this->get_element_style( 'title', array( 'color' => '#fff', 'font' => 'Helvetica', 'size' => '18px' ) ) . '">' . $this->get_title_setting( 'text', __( 'OptinMonster is the #1 lead generation platform for WordPress.', 'optin-monster-footer' ) ) . '</div>';
                // Form area
                ...
                $html .= '</span>';
                // Close button
                $html .= '<a href="#" class="om-close" title="' . esc_attr__( 'Close', 'optin-monster-footer' ) . '">&times;</a>';
            $html .= '</div>';
        $html .= '</div>';
    ...
    public function design_fields( $fields, $instance ) {
        ...
        $fields['body_bg'] = $instance->get_color_field(
            'body_bg',
            $instance->get_background_setting( 'body', '#000' ),
            __( 'Background Color', 'optin-monster-footer' ),
            __( 'The background color of the optin.', 'optin-monster-footer' ),
            '',
            array(
