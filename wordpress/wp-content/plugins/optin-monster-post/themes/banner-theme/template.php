<?php
/**
 * Builds out the post banner theme.
 *
 ...
 * @since        1.0.0
 */
class optin_monster_build_post_banner_theme {
	...
	public function build() {
		...
		html div#om-' . $this->hash . ' #om-post-' . $this->theme . '-content {
			position: relative;
			margin: 0 18px;
		...
		html div#om-' . $this->hash . ' #om-post-' . $this->theme . '-optin-title-container {
			position: relative;
		...
		html div#om-' . $this->hash . ' #om-post-' . $this->theme . '-optin-title:before,
		html div#om-' . $this->hash . ' #om-post-' . $this->theme . '-optin-title:after {
			content: "";
			position: absolute;
			display: block;
			border-style: solid;
		...
		$theme .= '<div id="om-' . $this->hash . '"><div id="om-post-' . $this->theme . '-optin" class="om-post-' . $this->theme . ' om-clearfix ' . $class . ' ' . ( isset( $this->meta['email']['provider'] ) && 'custom' == $this->meta['email']['provider'] ? 'om-custom-html-form' : '' ) . '">';
		    $title = $this->get_field( 'title' );
			$header_bg_color = $this->get_field( 'background', 'header' );
			$header_border_color = $this->get_field( 'header', 'border_color' );
		    $style = '';
			...
			if ( ! empty( $header_border_color ) )
				$style .= 'border-color:' . $header_border_color . ';';
		    if ( ! empty( $title['font'] ) )
			...
			$theme .= '<div id="om-post-' . $this->theme . '-content" class="om-clearfix" style="' .  ( ! empty( $body_bg_color ) ? 'background-color: ' . $body_bg_color : '' ) . '">';
					...
					$theme .= '<div id="om-post-' . $this->theme . '-right">';
						if ( isset( $this->meta['email']['provider'] ) && 'custom' == $this->meta['email']['provider'] ) :
							...
							if ( ! empty( $submit['bg_color'] ) )
								$style .= 'background-color:' . $submit['bg_color'] . ';border-color:' . $submit['border_color'] . ';';
