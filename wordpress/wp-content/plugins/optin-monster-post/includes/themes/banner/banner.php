...
class Optin_Monster_Post_Theme_Banner extends Optin_Monster_Theme {
	...
	public function get_styles() {
		...
		html div#om-' . $this->optin->post_name . ' #om-' . $this->type . '-' . $this->theme . '-header {
			...
			-moz-box-shadow: 0px 5px 11px -7px #000000;
			box-shadow: 0px 5px 11px -7px #000000;
			z-index: 2;
			background: url(' . plugins_url( 'images/header-border.png', __FILE__ ) . ') 0 5% repeat-x,
				url(' . plugins_url( 'images/header-border.png', __FILE__ ) . ') 0 95% repeat-x;
			background-color: #50aae9;
			border: 1px solid #3880bd;
		}
		html div#om-' . $this->optin->post_name . ' #om-' . $this->type . '-' . $this->theme . '-header:before,
		html div#om-' . $this->optin->post_name . ' #om-' . $this->type . '-' . $this->theme . '-header:after {
			content: "";
			position: absolute;
			display: block;
			border-style: solid;
			border-color: #3880bd transparent transparent transparent;
	...
	public function get_html() {
        ...
        $html = '<div id="om-' . $this->type . '-' . $this->theme . '-optin" class="om-' . $this->type . '-' . $this->theme . ' om-clearfix om-theme-' . $this->theme . ' ' . ( $provider && 'custom' == $provider ? 'om-custom-html-form' : '' ) . '">';
            $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-wrap" class="om-clearfix">';
                ...
                // Header area.
                $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-header" data-om-action="selectable" data-om-target="#optin-monster-field-header_bg" style="background-color:' . $this->get_background_setting( 'header', '#50aae9' ) . '">';
                    $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-title" data-om-action="editable" data-om-field="title" style="' . $this->get_element_style( 'title', array( 'color' => '#fff', 'font' => 'Oswald', 'size' => '24px', 'meta' => array( 'text-align' => 'center' ) ) ) . '">' . $this->get_title_setting( 'text', __( 'OptinMonster is the #1 lead generation platform for WordPress.', 'optin-monster-post' ) ) . '</div>';
                $html .= '</div>';

                // Content area.
                $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-content" class="om-clearfix" data-om-action="selectable" data-om-target="#optin-monster-field-content_bg" style="' . $this->get_background_setting( 'content', '#fff' ) . '">';
                    ...
                    $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-left" class="' . $image_class . '">';
						...
						$html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-image-container" class="om-image-container om-clearfix">';
							$html .= $this->get_image();
						$html .= '</div>';
                        $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-tagline" data-om-action="editable" data-om-field="tagline" style="' . $this->get_element_style( 'tagline', array( 'color' => '#858585', 'font' => 'Helvetica', 'size' => '15px' ) ) . '">' . $this->get_tagline_setting( 'text', __( 'OptinMonster allows you to explode your email list by using our incredible exit intent technology.', 'optin-monster-post' ) ) . '</div>';
                    $html .= '</div>';
	...
	public function design_fields( $fields, $instance ) {
		...
		$fields['header_bg'] = $instance->get_color_field(
			'header_bg',
			$instance->get_background_setting( 'header', '#50aae9' ),
		...
		$fields['header_border']  = $instance->get_hidden_field(
			'header_border',
			$instance->get_background_setting( 'header_border', '#3880bd' ),
			array(
				'target' => '#om-' . $this->type . '-' . $this->theme . '-header',
				'attr' => 'border-color',
				'method' => 'css',
	...
	public function input_fields( $fields, $instance ) {
        ...
        $fields['submit_bg'] = $instance->get_color_field(
            'submit_bg',
            $instance->get_submit_setting( 'bg_color', '#50aae9' ),
		...
		$fields['submit_border']  = $instance->get_hidden_field(
			'submit_border',
			$instance->get_submit_setting( 'border', '#3880bd' ),
			array(
				'target' => '#om-' . $this->type . '-' . $this->theme . '-optin-submit',
				'attr' => 'border-color',
				'method' => 'css',
	...
	public function save_controls( $meta, $optin_id, $fields, $post_data ) {
		...
		$meta['background']['header_border']  = isset( $fields['header_bg'] ) ? esc_attr( $fields['header_bg'] ) : '';
