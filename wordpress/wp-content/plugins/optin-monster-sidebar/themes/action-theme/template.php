<?php
/**
 * Builds out the sidebar action theme.
 *
 ...
 * @since        1.0.0
 */
class optin_monster_build_sidebar_action_theme {
	...
	public function build() {
        ...
        html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-tagline {
			font-size: 16px;
			line-height: 1.25;
			color: #484848;
			width: 100%;
		...
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-tagline span,
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-tagline strong,
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-tagline em,
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-tagline u {
			font-family: inherit;
		}
		...
		$theme .= '<div id="om-' . $this->hash . '"><div id="om-sidebar-' . $this->theme . '-optin" class="om-sidebar-' . $this->theme . ' om-clearfix ' . $class . ' ' . ( isset( $this->meta['email']['provider'] ) && 'custom' == $this->meta['email']['provider'] ? 'om-custom-html-form' : '' ) . '" style="' . $bg . '">';
		    ...
		    if ( ! empty( $tagline['meta'] ) )
		        foreach ( (array) $tagline['meta'] as $prop => $val )
		            $style .= str_replace( '_', '-', $prop ) . ':' . $val . ';';
			$theme .= '<p id="om-sidebar-' . $this->theme . '-optin-tagline" style="' . ( ! empty( $style ) ? $style : '' ) . '">' . ( ! empty( $tagline['text'] ) ? $tagline['text'] : '' ) . '</p>';
			...
			$theme .= 'function om_js_' . str_replace( '-', '_', $this->hash ) . '(){';
				...
				$theme .= 'this.resize_element = function($, el){';
					if ( 'customizer' == $this->env ) :
						...
						$theme .= '$("#om-sidebar-' . $this->theme . '-optin-name, #om-sidebar-' . $this->theme . '-optin-email").resize(function(){$("#om-sidebar-' . $this->theme . '-optin-submit").css({ "height": $(this).height() + "px", "line-height": ($(this).height()-10) + "px" });});';
