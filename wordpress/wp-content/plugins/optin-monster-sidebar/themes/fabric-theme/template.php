<?php
/**
 * Builds out the sidebar fabric theme.
 *
 ...
 * @since        1.0.0
 */
class optin_monster_build_sidebar_fabric_theme {
	...
	public function build() {
		...
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-top.om-has-image #om-sidebar-' . $this->theme . '-optin-content {
			width: 75%;
			padding-left: 15px;
		}
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-image-container {
			display: table-cell;
			vertical-align: middle;
			position: relative;
			width: 0%;
		...
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-top.om-has-image #om-sidebar-' . $this->theme . '-optin-image-container {
			width: 25%;
		}
		html div#om-' . $this->hash . ' #om-sidebar-' . $this->theme . '-optin-image-container img {
			display: block;
			margin: 0 auto;
			text-align: center;
		}
		...
		$theme .= '<div id="om-' . $this->hash . '"><div id="om-sidebar-' . $this->theme . '-optin" class="om-sidebar-' . $this->theme . ' om-clearfix ' . $class . ' ' . ( isset( $this->meta['email']['provider'] ) && 'custom' == $this->meta['email']['provider'] ? 'om-custom-html-form' : '' ) . '" style="' .  ( ! empty( $body_bg_color ) ? 'background-color: ' . $body_bg_color : '' ) . '">';
			...
			$theme .= '<div id="om-sidebar-' . $this->theme . '-content" class="om-clearfix">';
				$theme .= '<div id="om-sidebar-' . $this->theme . '-content-clear">';
					$image = get_the_post_thumbnail( $this->optin, 'full' ) ? ' om-has-image' : '';
					$theme .= '<div id="om-sidebar-' . $this->theme . '-top" class="' . $image . '">';
						...
						if ( ! empty( $content['meta'] ) )
							foreach ( (array) $content['meta'] as $prop => $val )
								$style .= str_replace( '_', '-', $prop ) . ':' . $val . ';';
						$theme .= '<div id="om-sidebar-' . $this->theme . '-optin-image-container">';
							$url = get_the_post_thumbnail( $this->optin, 'full' );
							$theme .= $url;
						$theme .= '</div>';
						$theme .= '<h2 id="om-sidebar-' . $this->theme . '-optin-content" class="' . $image . '" style="' . ( ! empty( $style ) ? $style : '' ) . '">' . ( ! empty( $content['text'] ) ? $content['text'] : '' ) . '</h2>';
					$theme .= '</div>';
