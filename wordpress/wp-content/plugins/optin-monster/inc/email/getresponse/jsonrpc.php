...
class jsonRPCClient
{
    ...
    public function __call($pMethod, $pParams)
    {
        ...
        $this->validate( ! $responseDecoded, __( 'There was an error decoding the JSON response. Please try again.', 'optin-monster' ) );
        ...
        {
            if ( isset($responseDecoded['error']['message'] ) && $responseDecoded['error']['message'] == 'Contact already queued for target campaign' ){
                $errorMessage = __('Please check your inbox for the confirmation email', 'optin-monster' );
            }
            else{
