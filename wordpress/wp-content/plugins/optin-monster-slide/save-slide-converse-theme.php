<?php
/**
 * Saves the slide optin "Converse Theme".
 *
 ...
 * @since        1.0.0
 */
class optin_monster_save_slide_converse_theme {
	...
	public function save_optin() {
		...
		// Save the optin title_closed.
		if ( empty( $this->meta['title_closed']['text'] ) )
		    $this->meta['title_closed']['text'] = isset( $this->data['optin_title_closed'] ) ? wp_kses_post( $this->data['optin_title_closed'] ) : '';
		else
		    $this->meta['title_closed']['text'] = isset( $this->data['optin_title_closed'] ) ? wp_kses_post( $this->data['optin_title_closed'] ) : $this->meta['title_closed']['text'];

		if ( empty( $this->meta['title_closed']['color'] ) )
		    $this->meta['title_closed']['color'] = isset( $this->data['optin_title_closed_color'] ) ? esc_attr( $this->data['optin_title_closed_color'] ) : '';
		else
		    $this->meta['title_closed']['color'] = isset( $this->data['optin_title_closed_color'] ) ? esc_attr( $this->data['optin_title_closed_color'] ) : $this->meta['title_closed']['color'];

		if ( empty( $this->meta['title_closed']['font'] ) ) {
		    $this->meta['title_closed']['font'] = isset( $this->data['optin_title_closed_font'] ) && 'Select your font...' !== $this->data['optin_title_closed_font'] ? esc_attr( $this->data['optin_title_closed_font'] ) : '';
		} else {
		    $this->meta['title_closed']['font'] = isset( $this->data['optin_title_closed_font'] ) && 'Select your font...' !== $this->data['optin_title_closed_font'] ? esc_attr( $this->data['optin_title_closed_font'] ) : $this->meta['title_closed']['font'];
		}

		if ( empty( $this->meta['title_closed']['size'] ) )
		    $this->meta['title_closed']['size'] = isset( $this->data['optin_title_closed_size'] ) ? esc_attr( $this->data['optin_title_closed_size'] ) : '';
		else
		    $this->meta['title_closed']['size'] = isset( $this->data['optin_title_closed_size'] ) ? esc_attr( $this->data['optin_title_closed_size'] ) : $this->meta['title_closed']['size'];

		// Save all meta related items.
		$this->save_meta( 'title_closed' );

		// Save the optin title_open.
		if ( empty( $this->meta['title_open']['text'] ) )
		    $this->meta['title_open']['text'] = isset( $this->data['optin_title_open'] ) ? wp_kses_post( $this->data['optin_title_open'] ) : '';
		else
		    $this->meta['title_open']['text'] = isset( $this->data['optin_title_open'] ) ? wp_kses_post( $this->data['optin_title_open'] ) : $this->meta['title_open']['text'];

		if ( empty( $this->meta['title_open']['color'] ) )
		    $this->meta['title_open']['color'] = isset( $this->data['optin_title_open_color'] ) ? esc_attr( $this->data['optin_title_open_color'] ) : '';
		else
		    $this->meta['title_open']['color'] = isset( $this->data['optin_title_open_color'] ) ? esc_attr( $this->data['optin_title_open_color'] ) : $this->meta['title_open']['color'];

		if ( empty( $this->meta['title_open']['font'] ) ) {
		    $this->meta['title_open']['font'] = isset( $this->data['optin_title_open_font'] ) && 'Select your font...' !== $this->data['optin_title_open_font'] ? esc_attr( $this->data['optin_title_open_font'] ) : '';
		} else {
		    $this->meta['title_open']['font'] = isset( $this->data['optin_title_open_font'] ) && 'Select your font...' !== $this->data['optin_title_open_font'] ? esc_attr( $this->data['optin_title_open_font'] ) : $this->meta['title_open']['font'];
		}

		if ( empty( $this->meta['title_open']['size'] ) )
		    $this->meta['title_open']['size'] = isset( $this->data['optin_title_open_size'] ) ? esc_attr( $this->data['optin_title_open_size'] ) : '';
		else
		    $this->meta['title_open']['size'] = isset( $this->data['optin_title_open_size'] ) ? esc_attr( $this->data['optin_title_open_size'] ) : $this->meta['title_open']['size'];

		// Save all meta related items.
		$this->save_meta( 'title_open' );
		...
		$this->meta['fonts'] = array( $this->meta['title_open']['font'], $this->meta['title_closed']['font'], $this->meta['tagline']['font'], $this->meta['name']['font'], $this->meta['email']['font'], $this->meta['submit']['font'] );
