...
<script type="text/javascript">
	...
	function backupbuddy_log( json ) {
		...
		if ( 'string' == ( typeof json ) ) {
			...
			message = json.date + " \t" + json.run + "sec \t" + json.mem + "MB\t" + json.data;
		}
	...
	function backupbuddy_poll() {
		...
		jQuery.ajax({
			...
			type:	'post',
			data:	{ serial: '<?php echo $serial_override; ?>', initwaitretrycount: backup_init_complete_poll_retry_count, action: 'pb_backupbuddy_backup_status' },
		...
		if ( '' !== backupbuddy_currentAction ) {
			...
			if ( 'cronPass' == backupbuddy_currentAction ) {
				if ( ( actionRunTime > 20 ) && ( sinceLastWarn > 45 ) ) { // sinceLastWarn is large number (timestamp) until set first time. so triggers off actionRunTime solely first.
					...
					thisSuggestion = {
						description: 'BackupBuddy uses WordPress\' scheduling system (cron) for running each backup step. Sometimes something inteferes with this scheduling, such as caching plugins.',
						quickFix: 'Narrow down the problem first.',
