...
// TODO: this currently only works for non-duplicatable features, this should eventually be moved to registerFeatureButtonEvents ( It will for sure need to be moved once this field type becomes avaible as a meta box )
    ...
    // if show feature is clicks, automatically open view all tab
    _.$('.show_feature a').click(function(){
      _.$('#developer-tools-nav .nav-tab').removeClass('nav-tab-active');
      _.$('#developer-tools-nav .nav-tab.all').addClass('nav-tab-active');
      _.showTabSection( 'all' );
      var $feature = _.$( '#' + _.$( this ).attr( 'class' ) );
    ...
    // man nav tabs
    _.$('#developer-tools-nav a.nav-tab').click( function(){
      _.$('#developer-tools-nav .nav-tab').removeClass('nav-tab-active');
       var tabSection = _.$( this ).attr( 'class' ).replace('nav-tab ', ''); // this is a bit hackish
      _.setCookie( 'developer_tools_current_menu_item', tabSection );
      _.showTabSection( tabSection );
      _.$( this ).addClass('nav-tab-active');
    });
    ...
    _.$( '.show_information' ).click( function(){
      ...
      if( $checkbox.is( ':checked' ) )
      {
        ...
        $feature.removeClass( 'hidden' );
        $featureInfo.removeClass( 'hidden' );
      }
      else
      {
